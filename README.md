...
The existing `apps/admin` now includes a **License / Key** section that embeds the original
NHATDUYPN license panel at `/license-panel/`.

- `apps/admin/public/license-panel` — original license management UI.
- `services/license-api` — original Cloudflare Worker API, adapted to run under `/license-api/*`.
- `supabase-license/migrations` — original Supabase schema/functions.
- `.github/workflows/6-deploy-license-api.yml` — optional deployment of the license Worker.

The portfolio API remains unchanged. The license system uses Supabase separately, so it does not
replace the existing D1/R2 backend.

Before deploying the license Worker, set these Worker secrets:
